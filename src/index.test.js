import { simulate_coding_framework } from './index';
const tests = [
  {
    input: [["FILE_UPLOAD", "Cars.txt", "200kb"],
    ["FILE_GET", "Cars.txt"],
    ["FILE_COPY", "Cars.txt", "Cars2.txt"],
    ["FILE_GET", "Cars2.txt"]],
    output: ["uploaded Cars.txt", "got Cars.txt", "copied Cars.txt to Cars2.txt", "got Cars2.txt"]
  }, {
    input: [["FILE_UPLOAD", "Foo.txt", "100kb"],
    ["FILE_UPLOAD", "Bar.csv", "200kb"],
    ["FILE_UPLOAD", "Baz.pdf", "300kb"],
    ["FILE_SEARCH", "Ba"]]
    , output: ["uploaded Foo.txt", "uploaded Bar.csv", "uploaded Baz.pdf", "found [Baz.pdf, Bar.csv]"]
  },
  {
    input: ["uploaded at Python.txt", "uploaded at CodeSignal.txt", "got at Python.txt", "copied at Python.txt to PythonCopy.txt", "found at [Python.txt, PythonCopy.txt]", "uploaded at Expired.txt", "file not found", "copied at CodeSignal.txt to CodeSignalCopy.txt", "found at [CodeSignal.txt, CodeSignalCopy.txt]"],
    output: [
    ...
    ], output: ["uploaded at Initial.txt", "uploaded at Update1.txt", "got at Initial.txt", "copied at Update1.txt to Update1Copy.txt", "uploaded at Update2.txt", "rollback to 2021-07-01T12:10:00", "got at Update1.txt", "got at Initial.txt", "found at [Update1.txt, Update1Copy.txt, Update2.txt]", "got at Update2.txt"]
  }
...
for (const [index, testCase] of tests.entries()) {
  test(`Exercise ${index + 1}`, () => {
    expect(simulate_coding_framework(testCase.input)).toEqual(testCase.output);
  })
}